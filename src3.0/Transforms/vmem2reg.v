...
(*****************************************************************************)
(* This file defines the variant mem2reg pass (vmem2reg) that is fully verified.
   See http://llvm.org/docs/Passes.html#mem2reg for the definition of LLVM's
   original mem2reg pass. *)

(*****************************************************************************)
(*  The following functions check if an alloca is legal to promote. An alloca   
    is promotable if its allocated location never escapes. Note that LLVM allows
    intrinsics use promotable alloca's, but vmem2reg disallows. *)
Definition is_promotable_cmd (pid:id) :=
  (fun (acc0:bool) (c:cmd) =>
...
Definition is_promotable_fun (pid:id) :=
  (fun (acc:bool) (b:block) =>
...
(* [cs]: the list of commands in the entry block of function [f]. 
   [dones]: the list of visited promotable variables.
   [find_promotable_alloca] finds the first promotable alloca in [cs],
     which is not in [dones]. *)
...
(* used by IterationPass.iter to do las/laa/sas in a function *)
Definition ElimStld := mkIterPass (list id) id elim_stld_block nil.

(* a flag to indicate if doing las/laa/sas elimination, realized by extraction.
   set by default. *)
Parameter does_stld_elim : unit -> bool.

(*********************************************************************)
(* remove stores to [pid] from [cs] *)
...
(* Check if [v0] is in the list of values [vs]. *)
Definition valueInListValueB (v0:value) (vs:list value) : bool :=
List.fold_left (fun acc v => acc || valueEqB v0 v) vs false.

(* Remove the values in [vs] that are already in [acc].
   Visited and unique values are stored in [acc].
 *) 
...
(* pruning when does_dead_phi_elim sets, not prunning by default *)
Parameter does_dead_phi_elim : unit -> bool.

(* Remove [pn] from [f] if [pn] is removable in terms of
     J. Aycock and N. Horspool. Simple Generation of Static Single Assignment Form. 
     In Compiler Construction (CC), 2000. 
   Returns the updated function and whether the function is changed.
*)
...
(* Remove redundant phinodes from [ps] *)
...
(* Remove redundant phinodes from [b]. [ut] and [dones] are only to 
   make the interface consistent with mkIterPass. *)
...
(* used by IterationPass.iter to do phi-elimination in a function *)
Definition ElimPhi := mkIterPass (list id) unit eliminate_block nil.

(* remove redundant phinodes when does_phi_elim sets, set by default *)
