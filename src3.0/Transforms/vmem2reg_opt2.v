...
(* vmem2reg-O1 addresses one kind of compile-time cost by ``fusing'' micro
passes. To address the other cost---the number of phi-nodes, we
implemented vmem2reg-O2 based on vmem2reg-O1.

vmem2reg-O2 places the minimal number of phi-nodes by the dominance-frontier 
algorithm. On average, the algorithm only introduces 1/8 phin-nodes of the 
pessimistic one, and does not need the additional AH phi-node elimination pass.

vmem2reg-O2 does not insert phinode-nodes at every block, so
LAS/LAA pairs may appear across blocks. To find them,
elim_stld_fdef extends the O1 algorthm by depth-first-searching functions' 
dominator trees.

Although vmem2reg-O2 has the same complexity as vmem2reg-O1, on average  
vmem2reg-O1 is 5>= times slower than mem2reg in terms of compile-time. To study 
the overhead cause by the purely functional programming style, we also 
implemented the C++ version of vmem2reg-O2. Because it uses constant-time 
hashtables, and does alias-based substitution, the C++ version's complexity 
is I. In practice, its compile-time is 1<= time of mem2reg's because we use a 
slightly efficient dominance-frontier calculation, and do not allow intrinsics 
to use promotable allocations. *)

(*********************************************************************)
(* Find domination frontiers *)
...
(* Find promotable allocas and store them in the initial definition maps
   used by generating IDF. *)
...
(*********************************************************************)
(* inserts indicates which block to insert phinodes
   scs: a list of successors
   The function returns true if one of scs needs to insert phinodes *)
...
(* generate fresh names.
   succs: successors of CFGs
   rd: reachable blocks
   inserts: where to insert phinodes
   ...
   If a block needs to place phinodes, we generate a fresh store at the 
   beginning. If one of a block's successors needs phinodes, the block needs
   a load at the end. *)
...
(************************************************)
(* A dominator tree for commands indiced by the label of the commands *)
...
(* Turn a list of blocks into a map of commands. *)
...
(* Given a dominator tree of labels, construct a tree of commands. *)
...
(************************************************)
(* Find LAS/LAA pairs on dom-trees. *)
