...
Definition INT_dec : forall x y : INT, {x=y} + {x<>y} := eq_nat_dec.
Definition sz_dec : forall x y : sz, {x=y} + {x<>y} := eq_nat_dec.
Definition align_dec : forall x y : align, {x=y} + {x<>y} := eq_nat_dec.
...
Definition szZERO : sz := 0.
Definition szONE : sz := 1.

Definition nat2sz (i:nat) : sz := i.
Definition sz2nat (i:sz) : nat := i.
Definition INT2nat (i:INT) : nat := i.
Definition nat2INT (i:nat) : INT := i.
...
| Cons_list_const idx idxs' => 
  ...
  | typ_struct lt => 
    match idx with
    | (const_int sz i) =>
      match (nth_list_typ (INT2nat i) lt) with
...
| insn_icmp _ _ _ _ _ => Some (typ_int szONE)
(* | insn_fcmp _ _ _ _ _ => Some (typ_int szONE) *)
...
Proof.
  ...
  destruct (@sz_dec s s0); try solve [subst; auto | done_right].

  destruct t2; try solve [done_right].
  destruct (@H t2); subst; try solve [done_right].
  destruct (@sz_dec s s0); subst; try solve [subst; auto | done_right].
...
Proof.
  ...
  destruct c2; try solve [done_right].
  destruct (@INT_dec i0 i1); try solve [done_right].
  destruct (@sz_dec s s0); try solve [subst; auto | done_right].
...
Proof.
  ...
  Case "insn_bop".
    ...
    destruct (@sz_dec s s0); subst; try solve [done_right].
  ...
  Case "insn_call".
    destruct (@id_dec i0 i2); subst; try solve [done_right]. 
    destruct (@id_dec i1 i3); subst; try solve [done_right]. 
...
Proof.
  destruct l1; destruct l2; try solve [subst; auto | done_right].
    destruct (@sz_dec s s0); subst; try solve [done_right].
    destruct (@align_dec a a1); subst; try solve [done_right].
    destruct (@align_dec a0 a2); subst; try solve [auto | done_right].
...
Fixpoint getTyp (c:const) : typ :=
 ...
 | const_arr lc => 
   match lc with
   | Nil_list_const => typ_array szZERO (typ_int szZERO)
   | Cons_list_const c' lc' => typ_array (nat2sz (length (unmake_list_const lc))) (getTyp c')
   end
