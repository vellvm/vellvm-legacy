...
(*
Inductive tree : Set :=
| def: forall (t:tree) (f:forest), tree
with forest : Set :=
| nil: forest
| cons: forall (t:tree) (f:forest), forest
.
...
Fixpoint tree_size (t:tree) : nat :=
  match t with
  | def t' f' => tree_size t' + forest_size f'
  end
...
Inductive tree : Set :=
| def: forall (t:tree) (f:list tree), tree
.

Definition forest := list tree.

Fixpoint tree_size (a:nat) (t:tree) : nat :=
  match t with
  | def t' f' => tree_size a t' + fold_left plus (map (tree_size a) f') 0
  end.

Fixpoint map2 (A B C:Type) (f:A->B->C) (la:list A) (lb:list B) : list C :=
match (la, lb) with
| (a::la', b::lb') => f a b :: map2 _ _ _ f la' lb'
| (_, _) => nil
end.

Fixpoint tree2_size (t:tree*tree) : nat :=
  match t with
  | (def t1' f1', def t2' f2') => 
    tree2_size (t1', t2') + 
    fold_left plus (map tree2_size (combine f1' f2')) 0
  end.

    fold_left plus (map2 _ _ _ tree2_size f1' f2') 0
...
Fixpoint typ_size (t:typ) : nat :=
  ...
  | typ_int n => 1
  | typ_metadata => 1
  | typ_function t1 lt2 => typ_size t1 + fold_left plus (map typ_size lt2) 0
  | typ_void => 1
  | typ_label => 1
...
Fixpoint typeEqB (tx ty:typ) {struct tx}: bool :=
match (tx, ty) with 
| (typ_int n, typ_int n') => 
...
(*  typeEqB t1 t1' &&  *)
  fold_left andb (@map typ bool typeEqB lt1 lt1') true
| (_, _) => false
end.

Fixpoint typeEqB (t t':typ) {struct t} : bool :=
match (t, t') with 
| (typ_int n, typ_int n') => 
  match (eq_nat_dec n n') with
  | left _ => true 
...
| (typ_function t1 lt1, typ_function t1' lt1') =>
  ...
  in
  typeEqB t1 t1' && 
  fold_left andb (map typeEqB_pair (combine lt1 lt1')) true
| (_, _) => false
end.

Definition insnEq (i1 i2:insn) : bool :=
...
(*
Fixpoint InInsn (i:insn) (li:list_insn) {struct li} : bool :=
match li with
| nil => false
| i' :: li' => b = a || InInsn i li
end.
*)
