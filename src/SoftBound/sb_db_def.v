...
Definition get_reg_metadata TD gl (rm:rmetadata) (v:value) 
    : option (metadata * typ) :=
  match v with
  | value_id pid => 
...
Definition SELECT TD v0 v1 v2 lc gl : option GenericValue :=
  match (getOperandValue TD v0 lc gl, getOperandValue TD v1 lc gl,
         getOperandValue TD v2 lc gl) with
...
| dbBop_error: forall TD rm lc gl id bop sz v1 v2 Mem MM als,
  BOP TD lc gl bop sz v1 v2 = None ->
...
| dbFBop_error: forall TD rm lc gl id fbop fp v1 v2 Mem MM als,
  FBOP TD lc gl fbop fp v1 v2 = None ->
...
| dbExtractValue_error : forall TD rm lc gl id t v gv Mem MM als idxs,
  getOperandValue TD v lc gl = None ->
...
| dbLoad_nptr : forall TD rm lc gl id t vp align Mem MM als gvp md gv mt,
  get_reg_metadata TD gl rm vp = Some (md,mt) ->
...
| dbStore_ptr : forall TD rm lc gl sid t v vp align Mem MM als gv gvp md mt Mem' 
  ...
  mstore TD Mem gvp t gv align = Some Mem' ->
  isPointerTyp t ->
  get_reg_metadata TD gl rm v = Some (md',mt') ->
...
| dbTrunc_error : forall TD rm lc gl id truncop t1 v1 t2 Mem MM als,
  TRUNC TD lc gl truncop t1 v1 t2 = None ->
...
| dbExt_error : forall TD rm lc gl id extop t1 v1 t2 Mem MM als,
  EXT TD lc gl extop t1 v1 t2 = None ->
...
| dbBitcast_ptr : forall TD rm lc gl id t1 v1 t2 gv2 Mem MM als lc' md rm' mt,
  CAST TD lc gl castop_bitcast t1 v1 t2 = Some gv2 ->
  isPointerTyp t1 ->
  get_reg_metadata TD gl rm v1 = Some (md,mt) ->
...
| dbCast_error : forall TD rm lc gl id castop t1 v1 t2 Mem MM als,
  CAST TD lc gl castop t1 v1 t2 = None ->
...
| dbIcmp_error : forall TD rm lc gl id cond t v1 v2 Mem MM als,
  ICMP TD lc gl cond t v1 v2 = None ->
...
| dbFcmp_error : forall TD rm lc gl id fcond fp v1 v2 Mem MM als,
  FCMP TD lc gl fcond fp v1 v2 = None ->
...
| dbSelect_nptr : forall TD rm lc gl id v0 t v1 v2 Mem MM als gv,
  SELECT TD v0 v1 v2 lc gl = Some gv ->
  ~ isPointerTyp t ->
...
| dbSelect_ptr : forall TD rm lc gl id v0 t v1 v2 Mem MM als gv md1 md2 cond lc' 
  ...
  SELECT TD v0 v1 v2 lc gl = Some gv ->
  isPointerTyp t ->
  get_reg_metadata TD gl rm v1 = Some (md1,mt1) ->
  get_reg_metadata TD gl rm v2 = Some (md2,mt2) ->
...
| dbSelect_error : forall TD rm lc gl id v0 t v1 v2 Mem MM als,
  SELECT TD v0 v1 v2 lc gl = None ->
  dbCmd TD gl lc rm als Mem MM (insn_select id v0 t v1 v2) lc rm als Mem MM
    trace_nil rerror

| dbLib : forall TD lc gl rid noret tailc ft fid rm MM
                          lp ft' Mem oresult Mem' lc' als r gvs,
  params2GVs TD lp lc gl = Some gvs ->
  SBSE.callLib Mem fid gvs = 
    Some (oresult, Mem', r) ->
...
| dbLib_error : forall TD lc gl rid noret tailc ft fid rm MM
                          lp ft' Mem oresult Mem' als r gvs,
  params2GVs TD lp lc gl = Some gvs ->
  SBSE.callLib Mem fid gvs = 
    Some (oresult, Mem', r) ->
  dbCmd TD gl
...
Fixpoint initRmetadata_aux TD gl (la:args) (lp:params) (rm accum:rmetadata) 
  : option rmetadata :=
match (la, lp) with
| ((t,_,id)::la', (_,v)::lp') =>
    if isPointerTypB t then
      ...
      | Some (md, mt) =>
        initRmetadata_aux TD gl la' lp' rm (updateAddAL _ accum id md)
      | _ => None
      end
    else initRmetadata_aux TD gl la' lp' rm accum
| (nil, nil) => Some accum
| _ => None
end.  

Definition initRmetadata TD gl (la:args) (lp:params) (rm:rmetadata) 
  : option rmetadata := 
  initRmetadata_aux TD gl la lp rm nil.
...
| dbCall_internal : forall S TD Ps lc rm gl fs rid noret tailc rt ft fv lp
  ...
  SBSE.isCall (insn_call rid noret tailc ft fv lp) = true ->
  callUpdateLocals TD noret rid oResult rm rm' lc lc' gl = 
    Some (lc'',rm'') ->
...
| dbCall_external : forall S TD Ps lc gl fs rid noret ca fv fid 
                          lp fa rt ft la va Mem MM oresult Mem' lc' rm gvs,
...
| dbCall_internal_error1 : forall S TD Ps lc rm gl fs rid noret tailc rt fv lp
  ...
  SBSE.isCall (insn_call rid noret tailc rt fv lp) = true ->
...
| dbCall_external_error1 : forall S TD Ps lc gl fs rid noret ca fv fid 
                          lp fa rt la va Mem MM oresult Mem' rm gvs,
  LLVMopsem.lookupExFdecViaGV TD Ps gl lc fs fv = 
    Some (fdec_intro (fheader_intro fa rt fid la va)) ->
  params2GVs TD lp lc gl = Some gvs ->
  LLVMopsem.callExternalFunction Mem fid gvs = 
...
| dbCall_external_error2 : forall S TD Ps lc gl fs rid noret tailc fv lp rt Mem 
                           MM rm,
  LLVMopsem.lookupExFdecViaGV TD Ps gl lc fs fv = None ->
  SBSE.isCall (insn_call rid noret tailc rt fv lp) = true ->
  dbCall S TD Ps fs gl lc rm Mem MM (insn_call rid noret tailc rt fv lp) lc rm 
    Mem MM trace_nil rerror
...
| dbFdef_func : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
...
| dbFdef_func_error1 : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
...
| dbFdef_func_error2 : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
                l1 ps1 cs1 tmn1 fa rt la va lb Result lc1 rm1 tr1 Mem MM Mem1 MM1
...
| dbFdef_func_error3 : forall S TD Ps gl fs fv fid lp lc rm rid
  ...
  getEntryBlock (fdef_intro (fheader_intro fa rt fid la va) lb) = 
    Some (block_intro l1 ps1 cs1 tmn1) ->
  initRmetadata TD gl la lp rm = None ->
...
| dbFdef_func_error5 : forall S TD Ps gl fs fv lp lc rm rid B rt Mem MM,
  LLVMopsem.lookupFdefViaGV TD Ps gl lc fs fv = None ->
  dbFdef fv rt lp S TD Ps lc rm gl fs Mem MM lc rm nil Mem MM B rid None 
    trace_nil rerror

| dbFdef_proc : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
...
| dbFdef_proc_error1 : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
                 l1 ps1 cs1 tmn1 fa rt la va lb lc1 rm1 tr1 Mem MM Mem1 MM1 als1
...
| dbFdef_proc_error2 : forall S TD Ps gl fs fv fid lp lc rm rm0 rid gvs
                 l1 ps1 cs1 tmn1 fa rt la va lb lc1 rm1 tr1 Mem MM Mem1 MM1 als1
                 l2 ps2 cs21 cs22 r,
  LLVMopsem.lookupFdefViaGV TD Ps gl lc fs fv = 
    Some (fdef_intro (fheader_intro fa rt fid la va) lb) ->
