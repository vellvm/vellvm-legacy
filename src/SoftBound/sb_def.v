...
Definition get_reg_metadata TD M gl (rm:rmetadata) (v:value) : option metadata :=
  match v with
  ...
  : option (GVMap * rmetadata):=
    ...
    | false =>
        ...
        | Some Result => 
          ...
          | Some gr => 
              do md' <- get_reg_metadata TD M gl rm' Result;
...
| ((t,id)::la', (_,v)::lp') =>
    if isPointerTypB t then
      do md <- get_reg_metadata TD Mem gl rm v;
...
Definition returnUpdateLocals (TD:TargetData) (M:mem) (c':cmd) (Result:value) 
  (lc lc':GVMap) (rm rm':rmetadata) (gl:GVMap) : option (GVMap * rmetadata) :=
    ...
    | Some id0 =>
        ...
        | (Some gr, Some md) => Some (prop_reg_metadata lc' rm' id0 gr md)
        | _ => None
        end
    | None => Some (lc', rm')
    end.
...
Fixpoint updateValuesForNewBlock 
  (ResultValues:list (id*GenericValue*option metadata)) (lc:GVMap) (rm:rmetadata)
  : GVMap * rmetadata :=
  ...
  dsInsn 
    (mk_sbState S TD Ps 
      ((mk_sbEC F B ((insn_call rid noret tailc rt fv lp)::cs) tmn lc rm als)
        ::EC) gl fs Mem MM)
    (mk_sbState S TD Ps 
      ((mk_sbEC (fdef_intro (fheader_intro rt fid la) lb) 
