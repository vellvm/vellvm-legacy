...
| (t0,v0) as p::lp' =>
    ...
    else trans_params ex_ids tmps rm lp' idx cs
end.
...
| insn_select id0 v0 t0 v1 v2 =>
    if isPointerTypB t0 then
      ...
      | (Some (mt1, bv1, ev1), Some (mt2, bv2, ev2), Some (bid0, eid0)) =>
          let '(btmp1,ex_ids) := mk_tmp ex_ids in
          let '(etmp1,ex_ids) := mk_tmp ex_ids in
          let '(btmp2,ex_ids) := mk_tmp ex_ids in
          let '(etmp2,ex_ids) := mk_tmp ex_ids in
          Some (ex_ids, btmp1::etmp1::btmp2::etmp2::tmps,
            c::
            insn_cast btmp1 castop_bitcast mt1 bv1 p8:: 
            insn_cast etmp1 castop_bitcast mt1 ev1 p8:: 
            insn_cast btmp2 castop_bitcast mt2 bv2 p8:: 
            insn_cast etmp2 castop_bitcast mt2 ev2 p8:: 
            insn_select bid0 v0 p8 (value_id btmp1) (value_id btmp2):: 
            insn_select eid0 v0 p8 (value_id etmp1) (value_id etmp2):: 
            nil, optaddrb, optaddre)
      | _ => None
      end
    else Some (ex_ids, tmps, [c], optaddrb, optaddre)
      ...
      (match v with
       | value_const (const_gid ft fid) =>
           if isSysLib fid then
              (Some (ex_ids, tmps, nil, 0%Z), v)
           else (trans_params ex_ids tmps rm p 1%Z nil, 
                 value_const (const_gid ft (wrapper_fid fid)))
       | _ => (trans_params ex_ids tmps rm p 1%Z nil, v)
      end) with
        ...
        let opt :=
          ...
          | insn_return _ t0 v0 =>
            if isPointerTypB t0 then
              ...
              Some (ex_ids',tmps',nil)
          | _ => Some (ex_ids',tmps',nil)
          end
        in
        match opt with
        | Some (ex_ids',tmps',cs') =>
