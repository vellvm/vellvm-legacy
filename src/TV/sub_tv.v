...
| (sterm_malloc sm t st1 a, sterm_malloc sm' t' st1' a') =>
    tv_smem fid sm sm' && tv_typ t t' && tv_sterm fid st1 st1' && tv_align a a'
...
| (sterm_lib sm i sts, sterm_lib sm' i' sts') =>
    tv_smem fid sm sm' && eq_id i i' && tv_list_sterm fid sts sts'
| _ => false
end
...
| (smem_malloc sm1 t1 st1 a1, smem_malloc sm2 t2 st2 a2) =>
    tv_smem fid sm1 sm2 && tv_typ t1 t2 && tv_sterm fid st1 st2 && tv_align a1 a2
| (smem_alloca sm1 t1 st1 a1, smem_alloca sm2 t2 st2 a2) =>
    if (tv_typ t1 t2 && tv_sterm fid st1 st2 && tv_align a1 a2)
    then tv_smem fid sm1 sm2
    else tv_smem fid (smem_alloca sm1 t1 st1 a1) sm2
...
| (smem_load sm1 t1 st1 a1, smem_load sm2 t2 st2 a2) =>
    if (tv_typ t1 t2 && tv_sterm fid st1 st2 && tv_align a1 a2)
    then tv_smem fid sm1 sm2
    else tv_smem fid (smem_load sm1 t1 st1 a1) sm2
| (smem_store sm1 t1 st11 st12 a1, smem_store sm2 t2 st21 st22 a2) =>
    tv_smem fid sm1 sm2 && tv_typ t1 t2 && tv_sterm fid st11 st21 &&
    tv_sterm fid st12 st22 && tv_align a1 a2
| (smem_lib sm1 fid1 sts1, smem_lib sm2 fid2 sts2) => 
    if (eq_id fid1 fid2 && tv_list_sterm fid sts1 sts2)
    then tv_smem fid sm1 sm2
    else tv_smem fid (smem_lib sm1 fid1 sts1) sm2
| (sm1, smem_lib sm2 fid sts) => smem_lib_sub fid && tv_smem fid sm1 sm2
| (sm1, smem_alloca sm2 t2 st2 a2) => tv_smem fid sm1 sm2
| (sm1, smem_load sm2 t2 st2 a2) => 
   ...
   *)
    tv_smem fid sm1 sm2 && load_from_metadata sm2 st2
| _ => false
end.
...
| (sframe_alloca sm1 sf1 t1 st1 a1, sframe_alloca sm2 sf2 t2 st2 a2) =>
    if (tv_smem fid sm1 sm2 && tv_typ t1 t2 && 
        tv_sterm fid st1 st2 && tv_align a1 a2)
    then tv_sframe fid sf1 sf2
    else tv_sframe fid (sframe_alloca sm1 sf1 t1 st1 a1) sf2
| _ => false
end.

Fixpoint tv_smap fid (sm1 sm2:smap) : bool :=
match sm1 with
| nil => true
| (id1,st1)::sm1' =>
  match lookupAL _ sm2 (rename_id fid id1) with
  | None => false
  | Some st2 => tv_sterm fid st1 st2 && tv_smap fid sm1' sm2
  end
end.

Definition sub_sstate fid s1 s2 := 
  tv_smap fid s1.(STerms) s2.(STerms) = true /\ 
  tv_smem fid s1.(SMem) s2.(SMem) = true /\
  tv_sframe fid s1.(SFrame) s2.(SFrame) = true /\ s1.(SEffects) = s2.(SEffects).

Lemma sstate_sub_dec : forall fid (sts1 sts2:sstate), 
  uniq sts1.(STerms) -> {sub_sstate fid sts1 sts2} + {~sub_sstate fid sts1 sts2}.
Proof.
Ltac done_right' := 
  right; intro J ; destruct J as [ J1 [J2 [J3 J4]]]; simpl in *; auto.

  intros fid sts1 sts2 Huniq.
  ...
  destruct (tv_smap fid st1 st2); auto.
  destruct (tv_smem fid sm1 sm2); auto.
  destruct (tv_sframe fid sf1 sf2); auto.
...
Definition tv_cmds fid (nbs1 nbs2 : list nbranch) :=
sumbool2bool _ _ 
  (sstate_sub_dec fid 
 ...
 * be the same. *)
Fixpoint tv_params fid (tsts1 tsts2:list (typ*sterm)) : bool :=
match (tsts1, tsts2) with
| (nil, _) => true
| ((t1,st1)::tsts1', (t2,st2)::tsts2') => 
  tv_typ t1 t2 && tv_sterm fid st1 st2 && tv_params fid tsts1' tsts2'
| _ => false
end.
...
Definition tv_subblock fid (sb1 sb2:subblock) :=
match (sb1, sb2) with
| (mkSB nbs1 call1 iscall1, mkSB nbs2 call2 iscall2) =>
  ...
  let cl2 := se_call st2 call2 iscall2 in
   (sumbool2bool _ _ (sstate_sub_dec fid st1 st2 (se_cmds_init_uniq nbs1)))
end.

Fixpoint tv_subblocks fid (sbs1 sbs2:list subblock) :=
match (sbs1, sbs2) with
| (nil, nil) => true
| (sb1::sbs1', sb2::sbs2') => 
    (tv_subblock fid sb1 sb2) && (tv_subblocks fid sbs1' sbs2')
| _ => false
end.
...
Definition tv_block fid (b1 b2:block) :=
match (b1, b2) with
| (block_intro l1 ps1 cs1 tmn1, block_intro l2 ps2 cs2 tmn2) =>
  match (cmds2sbs cs1, cmds2sbs cs2) with
  | ((sbs1, nbs1), (sbs2, nbs2)) =>
    eq_l l1 l2 && tv_phinodes fid ps1 ps2 && tv_subblocks fid sbs1 sbs2 &&
    tv_cmds fid nbs1 nbs2 && tv_terminator fid tmn1 tmn2
  end
end.

Fixpoint tv_blocks fid (bs1 bs2:blocks):=
match (bs1, bs2) with
| (nil, nil) => true
| (b1::bs1', b2::bs2') => tv_block fid b1 b2 && tv_blocks fid bs1' bs2'
| _ => false
end.
...
| (fdef_intro (fheader_intro _ fid1 _ as fh1) lb1, fdef_intro fh2 lb2) =>
  tv_fheader fh1 fh2 && tv_blocks fid1 lb1 lb2
end.
