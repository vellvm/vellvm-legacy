...
let rec string_of_constant c = 
  ...
  | LLVMsyntax.Coq_const_gep (ib, c, cs) -> 
		(
		match ib with
		| true ->  "gepelementptr inbounds ("^string_of_constant c^" "^string_of_list_constant cs^")"
		| false ->  "gepelementptr ("^string_of_constant c^" "^string_of_list_constant cs^")"
		)
  |	LLVMsyntax.Coq_const_select (c0, c1, c2) -> 
		"select ("^string_of_constant c0^" "^string_of_constant c1^" "^string_of_constant c2^")"	
  |	LLVMsyntax.Coq_const_icmp (cond, c1, c2) ->
		"icmp "^string_of_cond cond^" ("^string_of_constant c1^" "^string_of_constant c2^")"	
  |	LLVMsyntax.Coq_const_fcmp (cond, c1, c2) ->
		"fcmp "^string_of_fcond cond^" ("^string_of_constant c1^" "^string_of_constant c2^")"	
  ...
  | LLVMsyntax.Coq_const_insertvalue (c1, c2, cs) ->
		"insertvalue ("^string_of_constant c1^" "^string_of_constant c2^" "^string_of_list_constant cs^")"
...
let travel_cmd i =
  ...
  | LLVMsyntax.Coq_insn_fbop (id, fbop, fp, v1, v2) ->
		  eprintf "  %s = %s %s %s %s\n" id (string_of_fbop fbop) (string_of_floating_point fp) (string_of_value v1) (string_of_value v2)
  ...
  | LLVMsyntax.Coq_insn_trunc (id, truncop, t1, v, t2) ->
		  eprintf "  %s = %s %s %s %s\n" id (string_of_truncop truncop) (string_of_typ t1) (string_of_value v) (string_of_typ t2)
