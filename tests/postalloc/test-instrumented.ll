...
bb:		; preds = %bb, %entry
	...
	%arr.05_base = phi i8* [ null, %entry ], [ %bitcast, %bb ]		; <i8*> [#uses=1]
	%arr.05_bound = phi i8* [ null, %entry ], [ %bitcast1, %bb ]		; <i8*> [#uses=1]
	%safe.phi_node = phi i32 [ 0, %entry ], [ 1, %bb ]		; <i32> [#uses=0]
	%arr.05 = phi i32* [ null, %entry ], [ %.sub, %bb ]		; <i32*> [#uses=1]
	%value.03 = phi i32 [ 0, %entry ], [ %3, %bb ]		; <i32> [#uses=1]
	%1 = getelementptr i32* %arr.05, i32 %i.04		; <i32*> [#uses=2]
	%bcast_ld_dref_base = bitcast i8* %arr.05_base to i8*		; <i8*> [#uses=1]
	%arr.05_bound2 = bitcast i8* %arr.05_bound to i8*		; <i8*> [#uses=1]
	%bcast_ld_dref_bound = bitcast i32* %1 to i8*		; <i8*> [#uses=1]
	call void @__loadDereferenceCheck(i8* %bcast_ld_dref_base, i8* %arr.05_bound2, i8* %bcast_ld_dref_bound, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	%2 = load i32* %1, align 4		; <i32> [#uses=1]
	%3 = add i32 %2, %value.03		; <i32> [#uses=2]
	%4 = malloc [100 x i32]		; <[100 x i32]*> [#uses=3]
	%malloc_bound = getelementptr [100 x i32]* %4, i32 1		; <[100 x i32]*> [#uses=1]
	%bitcast = bitcast [100 x i32]* %4 to i8*		; <i8*> [#uses=1]
...
bb2:		; preds = %bb, %entry
	...
	%5 = tail call i32 (i8*, ...)* @printf(i8* noalias getelementptr ([4 x i8]* @.str1, i32 0, i32 0), i32 %value.0.lcssa) nounwind		; <i32> [#uses=0]
	ret void
}
