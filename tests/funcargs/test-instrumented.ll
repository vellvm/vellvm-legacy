...
@value = common global i32 0		; <i32*> [#uses=3]
...
entry:
	%value_bound2 = bitcast i8* %value_bound to i8*		; <i8*> [#uses=3]
	%value_base1 = bitcast i8* %value_base to i8*		; <i8*> [#uses=3]
...
bb:		; preds = %bb, %entry
	...
	%1 = getelementptr i32* %value, i32 %ptr.05.sum		; <i32*> [#uses=2]
	%bcast_ld_dref_base = bitcast i8* %value_base1 to i8*		; <i8*> [#uses=1]
	%bcast_arg_bound1 = bitcast i8* %value_bound2 to i8*		; <i8*> [#uses=1]
	%bcast_ld_dref_bound = bitcast i32* %1 to i8*		; <i8*> [#uses=1]
	call void @__loadDereferenceCheck(i8* %bcast_ld_dref_base, i8* %bcast_arg_bound1, i8* %bcast_ld_dref_bound, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
...
bb2:		; preds = %bb, %entry
	%value_addr.0.lcssa_base = phi i8* [ %value_base1, %entry ], [ %value_base1, %bb ]		; <i8*> [#uses=0]
	%value_addr.0.lcssa_bound = phi i8* [ %value_bound2, %entry ], [ %value_bound2, %bb ]		; <i8*> [#uses=0]
	%safe.phi_node = phi i32 [ 1, %entry ], [ 1, %bb ]		; <i32> [#uses=0]
	%value_addr.0.lcssa = phi i32* [ %value, %entry ], [ %3, %bb ]		; <i32*> [#uses=1]
	%5 = tail call i32 (i8*, ...)* @printf(i8* noalias getelementptr ([4 x i8]* @.str1, i32 0, i32 0), i32* %value_addr.0.lcssa) nounwind		; <i32> [#uses=0]
	ret void
}

declare void @test(i32, i32*) nounwind
