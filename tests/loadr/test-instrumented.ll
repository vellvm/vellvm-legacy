...
bb:		; preds = %bb, %entry
	...
	%ptr.05_base = phi i8* [ %bitcast, %entry ], [ %bitcast, %bb ]		; <i8*> [#uses=1]
	%ptr.05_bound = phi i8* [ %bitcast1, %entry ], [ %bitcast1, %bb ]		; <i8*> [#uses=1]
	...
	%2 = getelementptr i32* %ptr.05, i32 %i.04		; <i32*> [#uses=2]
	%bcast_ld_dref_base = bitcast i8* %ptr.05_base to i8*		; <i8*> [#uses=1]
	%ptr.05_bound2 = bitcast i8* %ptr.05_bound to i8*		; <i8*> [#uses=1]
	%bcast_ld_dref_bound = bitcast i32* %2 to i8*		; <i8*> [#uses=1]
	call void @__loadDereferenceCheck(i8* %bcast_ld_dref_base, i8* %ptr.05_bound2, i8* %bcast_ld_dref_bound, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
