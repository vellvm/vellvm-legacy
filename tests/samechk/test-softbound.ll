...
entry:
	%value = alloca i32, align 4		; <i32*> [#uses=13]
	%value1 = bitcast i32* %value to i32*		; <i32*> [#uses=1]
	%value2 = bitcast i32* %value to i8*		; <i8*> [#uses=7]
	%mtmp = getelementptr i32* %value1, i32 1		; <i32*> [#uses=1]
	%mtmp3 = bitcast i32* %mtmp to i8*		; <i8*> [#uses=7]
...
bb:		; preds = %bb, %entry
	...
	%2 = getelementptr i32* %value, i32 %ptr.04.sum6		; <i32*> [#uses=2]
	...
	%6 = getelementptr i32* %value, i32 %ptr.04.sum5		; <i32*> [#uses=2]
	%bcast_ld_dref_base8 = bitcast i8* %value2 to i8*		; <i8*> [#uses=1]
	%mtmp39 = bitcast i8* %mtmp3 to i8*		; <i8*> [#uses=1]
	%bcast_ld_dref_bound10 = bitcast i32* %6 to i8*		; <i8*> [#uses=1]
	call void @__loadDereferenceCheck(i8* %bcast_ld_dref_base8, i8* %mtmp39, i8* %bcast_ld_dref_bound10, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	%7 = load i32* %6, align 4		; <i32> [#uses=1]
	%8 = add i32 %7, %4		; <i32> [#uses=2]
	%bcast_st_dref_base11 = bitcast i8* %value2 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_bound12 = bitcast i8* %mtmp3 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_ptr13 = bitcast i32* %value to i8*		; <i8*> [#uses=1]
	call void @__storeDereferenceCheck(i8* %bcast_st_dref_base11, i8* %bcast_st_dref_bound12, i8* %bcast_st_dref_ptr13, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	store i32 %8, i32* %value, align 4
	%9 = add i32 %ptr.04.rec, 2		; <i32> [#uses=1]
	%ptr.04.sum = add i32 %9, %i.03		; <i32> [#uses=1]
	%10 = getelementptr i32* %value, i32 %ptr.04.sum		; <i32*> [#uses=2]
	%bcast_ld_dref_base14 = bitcast i8* %value2 to i8*		; <i8*> [#uses=1]
	%mtmp315 = bitcast i8* %mtmp3 to i8*		; <i8*> [#uses=1]
	%bcast_ld_dref_bound16 = bitcast i32* %10 to i8*		; <i8*> [#uses=1]
	call void @__loadDereferenceCheck(i8* %bcast_ld_dref_base14, i8* %mtmp315, i8* %bcast_ld_dref_bound16, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	%11 = load i32* %10, align 4		; <i32> [#uses=1]
	%12 = add i32 %11, %8		; <i32> [#uses=3]
	%bcast_st_dref_base17 = bitcast i8* %value2 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_bound18 = bitcast i8* %mtmp3 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_ptr19 = bitcast i32* %value to i8*		; <i8*> [#uses=1]
	call void @__storeDereferenceCheck(i8* %bcast_st_dref_base17, i8* %bcast_st_dref_bound18, i8* %bcast_st_dref_ptr19, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
...
bb2:		; preds = %bb, %entry
	...
	%16 = call i32 (i8*, ...)* @printf(i8* noalias getelementptr ([4 x i8]* @.str1, i32 0, i32 0), i32 %15) nounwind		; <i32> [#uses=0]
	ret void
}
