...
entry:
	...
	%1 = malloc [100 x i32]		; <[100 x i32]*> [#uses=4]
	...
	%.sub3 = getelementptr [100 x i32*]* %0, i32 0, i32 0		; <i32**> [#uses=6]
	%.sub = getelementptr [100 x i32]* %1, i32 0, i32 0		; <i32*> [#uses=3]
	%bcast_st_dref_base = bitcast i8* %bitcast to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_bound = bitcast i8* %bitcast1 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_ptr = bitcast i32** %.sub3 to i8*		; <i8*> [#uses=1]
	...
	%.sub3.vptr = bitcast i32** %.sub3 to i8*		; <i8*> [#uses=1]
	%bitcast3.vptr = bitcast i8* %bitcast3 to i8*		; <i8*> [#uses=1]
	%bitcast4.vptr = bitcast i8* %bitcast4 to i8*		; <i8*> [#uses=1]
	%.sub.aptr = bitcast i32* %.sub to i8*		; <i8*> [#uses=1]
	call void @__hashStoreBaseBound(i8* %.sub3.vptr, i8* %bitcast3.vptr, i8* %bitcast4.vptr, i8* %.sub.aptr, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	%2 = icmp sgt i32 %mm, 0		; <i1> [#uses=1]
	br i1 %2, label %bb, label %bb2

bb:		; preds = %bb, %entry
	%i.04 = phi i32 [ 0, %entry ], [ %indvar.next5, %bb ]		; <i32> [#uses=2]
	%3 = getelementptr [100 x i32]* %1, i32 0, i32 %i.04		; <i32*> [#uses=3]
...
bb1.bb2_crit_edge:		; preds = %bb
	%bcast_st_dref_base8 = bitcast i8* %bitcast to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_bound9 = bitcast i8* %bitcast1 to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_ptr10 = bitcast i32** %.sub3 to i8*		; <i8*> [#uses=1]
	...
	%.sub3.vptr5 = bitcast i32** %.sub3 to i8*		; <i8*> [#uses=1]
	%bitcast3.vptr6 = bitcast i8* %bitcast3 to i8*		; <i8*> [#uses=1]
	%bitcast4.vptr7 = bitcast i8* %bitcast4 to i8*		; <i8*> [#uses=1]
	%.aptr = bitcast i32* %3 to i8*		; <i8*> [#uses=1]
	call void @__hashStoreBaseBound(i8* %.sub3.vptr5, i8* %bitcast3.vptr6, i8* %bitcast4.vptr7, i8* %.aptr, i32 ptrtoint (i32* getelementptr (i32* null, i32 1) to i32), i32 1)
	br label %bb2

bb2:		; preds = %bb1.bb2_crit_edge, %entry
	%_base = phi i8* [ %bitcast3, %bb1.bb2_crit_edge ], [ %bitcast3, %entry ]		; <i8*> [#uses=0]
	%_bound = phi i8* [ %bitcast4, %bb1.bb2_crit_edge ], [ %bitcast4, %entry ]		; <i8*> [#uses=0]
	%safe.phi_node = phi i32 [ 1, %bb1.bb2_crit_edge ], [ 1, %entry ]		; <i32> [#uses=0]
	%4 = phi i32* [ %3, %bb1.bb2_crit_edge ], [ %.sub, %entry ]		; <i32*> [#uses=1]
	%5 = tail call i32 (i8*, ...)* @printf(i8* noalias getelementptr ([4 x i8]* @.str1, i32 0, i32 0), i32* %4) nounwind		; <i32> [#uses=0]
	ret void
}
