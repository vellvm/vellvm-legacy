...
entry:
	...
	%bitcast = bitcast [100 x i32]* %0 to i8*		; <i8*> [#uses=4]
	%bitcast1 = bitcast [100 x i32]* %malloc_bound to i8*		; <i8*> [#uses=4]
...
bb:		; preds = %bb, %entry
	...
	%ptr.04_base = phi i8* [ %bitcast, %entry ], [ %bitcast, %bb ]		; <i8*> [#uses=1]
	%ptr.04_bound = phi i8* [ %bitcast1, %entry ], [ %bitcast1, %bb ]		; <i8*> [#uses=1]
	%safe.phi_node = phi i32 [ 1, %entry ], [ 1, %bb ]		; <i32> [#uses=0]
	%ptr.04 = phi i32* [ %.sub, %entry ], [ %4, %bb ]		; <i32*> [#uses=1]
	%2 = getelementptr i32* %ptr.04, i32 %i.03		; <i32*> [#uses=2]
	%bcast_st_dref_base = bitcast i8* %ptr.04_base to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_bound = bitcast i8* %ptr.04_bound to i8*		; <i8*> [#uses=1]
	%bcast_st_dref_ptr = bitcast i32* %2 to i8*		; <i8*> [#uses=1]
...
bb2:		; preds = %bb, %entry
	%ptr.0.lcssa_base = phi i8* [ %bitcast, %entry ], [ %bitcast, %bb ]		; <i8*> [#uses=0]
	%ptr.0.lcssa_bound = phi i8* [ %bitcast1, %entry ], [ %bitcast1, %bb ]		; <i8*> [#uses=0]
	%safe.phi_node2 = phi i32 [ 1, %entry ], [ 1, %bb ]		; <i32> [#uses=0]
	%ptr.0.lcssa = phi i32* [ %.sub, %entry ], [ %4, %bb ]		; <i32*> [#uses=1]
	%5 = tail call i32 (i8*, ...)* @printf(i8* noalias getelementptr ([4 x i8]* @.str1, i32 0, i32 0), i32* %ptr.0.lcssa) nounwind		; <i32> [#uses=0]
	ret void
}
